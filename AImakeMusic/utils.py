...
def convert_midi_to_mp3():
    ...
    """
    command = 'timidity {} -Ow -o {}'.format(input_file, output_wav_file)
    return_code = subprocess.call(command, shell=True)

    if return_code != 0:
        print('转换时出错，请查看出错信息')
    else:
        print('转换完毕. 生成的文件是 {}'.format(output_wav_file))

    command = 'ffmpeg -i {} -acodec libmp3lame -ab 256k {}'.format(output_wav_file, output_file)
    return_code = subprocess.call(command, shell=True)
