...
def test(model_path, test_data, vocab_size, id_to_word):
    # 测试的输入
    test_input = utils.Input(batch_size=20, num_steps=35, data=test_data)
    ...
    m = network.Model(test_input, is_training=False, hidden_size=650, vocab_size=vocab_size, num_layers=2)
...
if __name__ == "__main__":
    ...
    if utils.args.load_file:
        load_file = utils.args.load_file
    train_data, valid_data, test_data, vocab_size, id_to_word = utils.load_data(data_path)

    trained_model = utils.save_path + "/" + load_file
