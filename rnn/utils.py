...
parser.add_argument('--data_path', type=str, default=data_path,
                    help='The path of the data for training and testing')  # 参数--data_path,数据类型str，默认的路径data_path help帮助说明
# 测试时读取模型参数文件的名称
...
# 如果是 Python3 版本
Py3 = sys.version_info[0] == 3  # 如果系统中python中的版本号的开头为3，那么就是py3
...
# 生成批次样本
# raw_data传进去的数据，batch_size,num_steps
def generate_batches(raw_data, batch_size, num_steps):
    # 将数据转为 Tensor 类型
    raw_data = tf.convert_to_tensor(raw_data, name="raw_data", dtype=tf.int32)

    data_len = tf.size(raw_data)  # raw_data的长度
