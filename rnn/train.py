...
def train(train_data, vocab_size, num_layers, num_epochs, batch_size, model_save_name,
    ...
    # 训练的输入
    training_input = utils.Input(batch_size=batch_size, num_steps=35, data=train_data)

    # 创建训练的模型
    m = network.Model(training_input, is_training=True, hidden_size=650, vocab_size=vocab_size, num_layers=num_layers)
        ...
        for epoch in range(num_epochs):
            ...
            for step in range(training_input.epoch_size):
                ...
                if step % print_iter != 0:
                    ...
                    curr_time = datetime.datetime.now()
                    cost, _, current_state, acc = sess.run([m.cost, m.train_op, m.state, m.accracy],
                                                           feed_dict={m.init_state: current_state})
                    # 每 print_iter（默认是 50）打印当下的 Cost（误差/损失）和 Accuracy（精度）
                    print("Epoch {},第 {} 步，损失: {:.3f}, 每步所用秒数：{:.2f}".format(epoch, step, cost, acc, seconds))

            # 保存一个模型的变量的 checkpoint 文件
            saver.save(sess, utils.save_path + '/' + model_save_name, global_step=epoch)
        # 对模型做一次总的保存
        saver.save(sess, utils.save_path + '/' + model_save_name + '-final')
...
if __name__ == "__main__":
    if utils.args.data_path:
        data_path = utils.args.data_path
    train_data, valid_data, test_data, vocab_size, id_to_word = utils.load_data(data_path)
