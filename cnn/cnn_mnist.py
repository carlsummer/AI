...
# 计算误差（先用 Softmax激活函数 计算百分比概率，
# 再用 Cross entropy（交叉熵）来计算百分比概率和对应的独热码之间的误差）
loss = tf.losses.softmax_cross_entropy(
    onehot_labels=output_y,  # 编码标签
    logits=logits  # 卷积神经网络的输出
)
...
# 返回 (accuracy 精度, update_op 更新的操作), 会创建两个 局部变量
accuracy = tf.metrics.accuracy(
    labels=tf.argmax(output_y, axis=1),  # 真实的标签,tf.argmax 返回最大值的下标
    predictions=tf.argmax(logits, axis=1),  # 预判的
)[1]
...
# 训练 5000 步。这个步数可以调节
for i in range(100):
    batch = mnist.train.next_batch(50)  # batch 一批  # 从 Train（训练）数据集里取 “下一个” 50 个样本
